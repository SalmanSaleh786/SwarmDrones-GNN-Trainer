...
# Load test dataset
train_data_list = torch.load("graphs_dataset_for loss 0.99.pt")
train_data_list = [
    data for data in train_data_list
    if data.edge_index.dim() == 2 and data.edge_index.size(0) == 2 and data.edge_index.numel() > 0
]
...
with torch.no_grad():  # Disable gradient tracking for testing
    for test_graph in train_data_list:
